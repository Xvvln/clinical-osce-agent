...
test("report page shows submitted material coverage map as the round conclusion", () => {
  ...
  assert.match(reportSource, /const trainingProgress = report\.training_progress_snapshot;/);
  assert.match(reportSource, /const coverageMap = trainingProgress\?\.coverage_map;/);
  ...
  assert.match(reportSource, /素材探索/);
  assert.match(reportSource, /必做查体与检查/);
  assert.match(reportSource, /item\.status === "covered" \? "已触发" : "本轮未触发"/);
  ...
  assert.match(reportSource, /\.\.\.report\.next_recommendations\.map/);
  assert.match(summarySource, /\.slice\(0, 3\);/);
  assert.match(summarySource, /本轮素材探索明细/);
  assert.match(summarySource, /问诊/);
  assert.match(summarySource, /查体/);
  assert.match(summarySource, /检查/);
  assert.match(summarySource, /推理/);
