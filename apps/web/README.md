...
学生端已关闭 Next.js 开发模式自带悬浮 indicator，并实现自有 OSCE Dock：圆形入口默认位于左下角，可自由拖动，松手后自动吸附到左右屏幕边缘；展开后可进入病例、记录、画像、安全、来源、报告、过程提示和患者信息等快捷操作。该入口只调用既有学生端页面和训练 API，不参与诊断裁判或评分逻辑。
