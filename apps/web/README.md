...
学生端已关闭 Next.js 开发模式自带悬浮 indicator，并实现自有 OSCE Dock：圆形入口默认位于左下角，可自由拖动，松手后自动吸附到左右屏幕边缘；展开后为白色面板，并通过“训练 / 资料 / 系统”二级菜单进入病例库、评分报告、过程提示、患者信息、安全声明、数据来源和管理端 API 配置。该入口只调用既有学生端页面和训练 API，不参与诊断裁判或评分逻辑。
