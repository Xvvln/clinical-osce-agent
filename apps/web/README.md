...
学生端已关闭 Next.js 开发模式自带悬浮 indicator，并实现自有 OSCE Dock：圆形入口默认位于左下角，按钮内部带白色镂空圆环，可自由拖动，松手后自动吸附到左右屏幕边缘；展开后默认只显示白色一级菜单，一级菜单按“训练操作台 / 系统与配置 / 资料与说明 / 关闭菜单”排列，点击前三项才会在侧边继续展开对应子菜单，进入病例库、评分报告、过程提示、患者信息、安全声明和数据来源。当前比赛 / 测试构建采用服务端统一托管模型配置，`API 配置` 为只读说明弹窗，学生端不提供 provider 选择、API Key 输入、保存或连通性测试，也不会把模型密钥写入浏览器。底层 `/api/model-config/test` 仅保留给显式启用账号级配置的 `local-dev` / `local-demo` 环境：要求普通用户登录但不要求管理员角色，匿名请求返回 401 且不会发出上游探针；服务端托管模式及生产模式返回 403。启用账号级配置后，配置只在当前账号的单次训练 / 报告请求上下文中生效，状态查询不会把某个账号切换成整个进程的默认配置；后台报告增强会按 session 所属账号重新绑定。账号级 Vertex ADC 只允许直连，需要代理时改用 Vertex API Key 或服务端托管代理。规则评分、病例标准答案和诊断裁判仍由后端确定性控制。
