...
学生端已关闭 Next.js 开发模式自带悬浮 indicator，并实现自有 OSCE Dock：圆形入口默认位于左下角，按钮内部带白色镂空圆环，可自由拖动，松手后自动吸附到左右屏幕边缘；展开后默认只显示白色一级菜单，一级菜单按“训练操作台 / 系统与配置 / 资料与说明 / 关闭菜单”排列，点击前三项才会在侧边继续展开对应子菜单，进入病例库、评分报告、过程提示、患者信息、安全声明和数据来源。`API 配置` 会在学生端白色弹窗内选择自定义后端、Gemini Developer API、Vertex Gemini ADC、Vertex Gemini API Key、OpenAI 兼容或 Anthropic 服务端；Gemini Developer API 与 Vertex Gemini ADC 默认模型为 `gemini-3.1-pro-preview`，Vertex Gemini API Key / Express Mode 默认模型为 `gemini-2.5-flash`，Vertex Gemini ADC 不需要 API Key，只填写 Project ID、模型和代理，Vertex Gemini API Key 不需要 Project ID。OpenAI 兼容、Anthropic、Vertex Gemini ADC 或 Vertex Gemini API Key 配置保存时会调用 `/api/model-config/runtime`，运行时模型配置按当前登录账号持久化，并可用于标准化病人、`llm_rubric` 和 Skill 候选文案生成；前端不会把密钥写入浏览器 `localStorage`，后端状态接口也不回显密钥，只返回是否已保存密钥，后续同一账号可留空沿用。`/api/model-config/test` 可测试连通性：自定义后端测试 `/health`，Gemini Developer API 测试 `/v1beta/models`，Vertex Gemini ADC 通过本机 ADC 发起最小 Gemini 调用，Vertex Gemini API Key 通过 Google Gen AI SDK 发起最小 Gemini 调用，OpenAI 兼容服务端发出最小 `/chat/completions` 探针，Anthropic 服务端发出最小 `/v1/messages` 探针。规则评分、病例标准答案和诊断裁判仍由后端确定性控制。
