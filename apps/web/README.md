...
工作台问诊输入栏已接入语音输入：浏览器录音会上传到 `/api/audio/transcriptions` 转写，并只填入输入框，不自动发送。患者回复区提供可持久化的“文本模式 / 语音模式”：文本模式不自动请求 TTS，语音模式在每条新标准化病人回复完整显示后自动调用 `/api/audio/speech` 并播放；两种模式都保留逐条手动播放按钮。前端会在用户选择语音模式或发送问诊时预激活 Web Audio，浏览器仍拒绝自动播放时则提示点击播放，不中断文字训练。阿里云 DashScope key 只放在 API 服务端环境变量中，前端不保存语音密钥。
