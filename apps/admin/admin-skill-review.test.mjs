...
test("admin dashboard reads management data and exposes review actions", () => {
  ...
  assert.match(adminPageSource, /训练前可用于教学提示/);
  assert.match(adminPageSource, /仅提交后复盘/);
  assert.match(adminPageSource, /仅管理员可见/);
  assert.match(adminPageSource, /标准答案保护区/);
  assert.match(adminPageSource, /关联来源（可选）/);
  assert.match(adminPageSource, /toggleAdminCsvToken/);
