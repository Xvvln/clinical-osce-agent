...
- **RAG 教学知识库**：教师可维护全局或病例知识库，并按训练阶段、可见性和可用 Agent 约束检索；知识只用于提示、复盘、Skill 生成 / 审批和来源追溯，不参与诊断裁判或 rubric 评分。
...
- **知识库**：上传或编辑全局 / 病例知识文档，并控制适用训练阶段、可见性和可用 Agent；MD、TXT、CSV、HTML、PDF、DOCX、PPTX 和常见图片均可在项目内直接解析，扫描 PDF 与图片内文字由本地 OCR 识别。
